...
function serviceText(s){return `${s.exact_service_name||''} ${s.source_category||''} ${s.quality||''}`.toLowerCase();}
...
function isLowPopularity(s){
  ...
  return /low\s*popularity|low\s*popular|low\s*demand|not\s*popular|unpopular|poor\s*popularity/.test(t);
}
function isBestSeller(s){
  const t=serviceText(s);
  return /best\s*seller|bestseller|best-selling|most\s*popular|top\s*seller/.test(t);
}
function isTargeted(s){
  const t=serviceText(s);
  return /targeted|target\b|geo\s*target|country\s*target|audience\s*target/.test(t);
}
function isHQ(s){
  const t=serviceText(s);
  return /\bhq\b|high\s*quality|premium\s*quality|real\s*quality/.test(t);
}
...
function desirabilityScore(s,medianPrice){
  let score=0;
  if(s.health_status==='green')score+=35; else if(s.health_status==='yellow')score+=10; else score-=60;
  if(isBestSeller(s))score+=24;
  if(isTargeted(s))score+=20;
  if(isHQ(s))score+=16;
  if(String(s.function_label).toUpperCase()==='LIKES' && medianPrice && Number.isFinite(Number(s.current_price))){
    const ratio=Number(s.current_price)/medianPrice;
    if(ratio>=0.70 && ratio<=1.35)score+=14;
    else if(ratio>=0.45 && ratio<=1.75)score+=5;
    else score-=4;
  }
  const start=Number(s.avg_start_seconds);
  if(Number.isFinite(start)) score += Math.max(-20,12-(start/300));
  const completion=Number(s.avg_completion_seconds);
  if(Number.isFinite(completion)) score += Math.max(-15,10-(completion/1800));
...
function renderServiceArea(data,cat,sort='best'){
  ...
  const eligible=[...data].filter(s=>isInstagramService(s) && !isLowPopularity(s));
  const median=categoryMedianPrice(eligible);
  let sorted=sort==='price'
    ? eligible.sort(comparePrice).slice(0,15)
    : eligible.sort((a,b)=>desirabilityScore(b,median)-desirabilityScore(a,median) || comparePrice(a,b)).slice(0,15);
...
function serviceCard(s){
  ...
  return `<article class="service" data-service="${s.service_id}">
    <div class="service-head">
      ...
      <div><div class="service-title">${esc(s.function_label)} · ${esc(s.panel_code)} · ID ${esc(s.service_code)} <span class="statusdot ${esc(s.health_status)}"></span>${isTargeted(s)?'<span class="tag">TARGETED</span>':''}${isHQ(s)?'<span class="tag">HQ</span>':''}${isBestSeller(s)?'<span class="tag hot">BEST SELLER</span>':''}</div><div class="small">Start ${sec(s.avg_start_seconds)}</div></div>
